...
fields <- c("jobs", "education", "teaching", "service", "research", "skills")
...
all_data[['publications']] %>%
  filter(str_starts(author, "BP Labbree")|str_starts(author,"B Labbree")) %>%
  arrange(desc(year)) %>%
  # mutate(author = parse_authors(author)) %>%
  detailed_entries(
    what=author, 
    when=year, 
    with=title, 
